...
from libs.SDK.SendTemplateSMS import ccp
import tornado.gen
...
class SMSCodeHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not all((mobile, piccode, piccode_id)):
            raise tornado.gen.Return(self.write(dict(errcode=RET.PARAMERR, errmsg="参数缺失")))
        if not re.match(r"^1\d{10}$", mobile):
            raise tornado.gen.Return(self.write(dict(errcode=RET.PARAMERR, errmsg="手机号格式错误")))
        ...
        try:
            ...
            logging.error(e)
            raise tornado.gen.Return( self.write(dict(errcode=RET.DBERR, errmsg="查询验证码错误")))

        if not real_piccode:
            raise tornado.gen.Return( self.write(dict(errcode=RET.NODATA, errmsg="验证码过期")))
        ...
        if real_piccode.lower() != piccode.lower():
            raise tornado.gen.Return( self.write(dict(errcode=RET.DATAERR, errmsg="验证码错误")))
        ...
        try:
            ...
            if 0 != ret["counts"]:
                raise tornado.gen.Return( self.write(dict(errcode=RET.DATAEXIST, errmsg="手机号已注册")))
        ...
        try:
            ...
            logging.error(e)
            raise tornado.gen.Return( self.write(dict(errcode=RET.DBERR, errmsg="数据库出错")))

        # 发送短信验证码
        try:
            result =yield ccp.sendTemplateSMS(mobile, [sms_code, SMS_CODE_EXPIRES_SECONDS/60], 1)
        except Exception as e:
            logging.error(e)
            raise self.write(dict(errcode=RET.THIRDERR, errmsg="发送短信失败"))
        if result:
        ...
        # except Exception as e:
        #     logging.error(e)
        #     return self.write(dict(errcode=RET.DATAERR,errmsg="数据查询错误"))
        # if not image_code_text:
        #     return  self.write(dict(errcode=RET.NODATA,errmsg="图片验证码已过期"))
        # if not real_image_code_text.lower()!=image_code_text.lower():
        ...
        # try:
        #     restult=ccp.sendTemplateSMS(mobile,[sms_code, SMS_CODE_EXPIRES_SECONDS/60],1)
        # except Exception as e:
        #     logging.error(e)
